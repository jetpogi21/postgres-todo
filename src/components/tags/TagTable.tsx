...
const TagTable = <T,>({
  ...
  */
  ...
  const handleSubmit = async (values: TagFormikInitialValues, formik: FormikHelpers<TagFormikInitialValues>) => {
    ...
    asyncUpdateRecords(payload).then((data) => {
      const { inserted, updated } =
        data as unknown as UpdateModelsData<TagModel>;
      ...
      Object.keys(inserted).forEach((idx) => {
        ...
        formik.setFieldValue(`${pluralizedModelName}[${idx}]`, {
          ...
          touched: false,
          [primaryKeyFieldName]:
            inserted[numIdx][primaryKeyFieldName as keyof TagModel],
        });
      });
  ...
  const openDialogHandler = (row?: Row<T>["original"]) => {
    openDialog({
      ...
      message: (
        <div className="pt-8">
          <TagForm
            data={(row ? row : null) as TagModel | null}
            id={
              row
