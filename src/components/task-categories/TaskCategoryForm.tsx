...
"use client";
import {
  TaskCategoryFormFormikInitialValues,
  TaskCategoryModel,
...
interface TaskCategoryFormProps {
  data: TaskCategoryModel | null;
  id: string;
  modalFormProps?: ModelFormProps;
  hiddenField?: string;
  onSubmit?: (
    values: TaskCategoryFormFormikInitialValues,
    formik: FormikHelpers<TaskCategoryFormFormikInitialValues>
  ) => void;
}
...
const TaskCategoryForm: React.FC<TaskCategoryFormProps> = (prop) => {
  ...
  const taskCategory = modelQuery.data as TaskCategoryFormFormikInitialValues;

  const isLarge = useScreenSize("lg");
  const initialValues = getInitialValues<TaskCategoryFormFormikInitialValues>(
    modelConfig,
    taskCategory,
  ...
  const handleFormikSubmit = (
    ...
    const goToNewRecord = () => {
      formik.setValues(
        getInitialValues<TaskCategoryFormFormikInitialValues>(
          modelConfig,
          undefined,
          { requiredList }
        )
    ...
    //e.g. { deletedTaskCategoryTags: [], newTags: []}
  ...
  const renderFormik = (
    formik: FormikProps<TaskCategoryFormFormikInitialValues>
  ) => {
