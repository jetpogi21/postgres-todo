...
"use client";
import {
  TaskIntervalFormFormikInitialValues,
  TaskIntervalModel,
...
interface TaskIntervalFormProps {
  data: TaskIntervalModel | null;
  id: string;
  modalFormProps?: ModelFormProps;
  hiddenField?: string;
  onSubmit?: (
    values: TaskIntervalFormFormikInitialValues,
    formik: FormikHelpers<TaskIntervalFormFormikInitialValues>
  ) => void;
}
...
const TaskIntervalForm: React.FC<TaskIntervalFormProps> = (prop) => {
  ...
  const taskInterval = modelQuery.data as TaskIntervalFormFormikInitialValues;

  const isLarge = useScreenSize("lg");
  const initialValues = getInitialValues<TaskIntervalFormFormikInitialValues>(
    modelConfig,
    taskInterval,
  ...
  const handleFormikSubmit = (
    ...
    const goToNewRecord = () => {
      formik.setValues(
        getInitialValues<TaskIntervalFormFormikInitialValues>(
          modelConfig,
          undefined,
          { requiredList }
        )
    ...
    //e.g. { deletedTaskIntervalTags: [], newTags: []}
  ...
  const renderFormik = (
    formik: FormikProps<TaskIntervalFormFormikInitialValues>
  ) => {
...
export default TaskIntervalForm;
