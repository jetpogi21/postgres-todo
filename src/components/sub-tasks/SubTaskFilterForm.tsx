...
const SubTaskFilterForm = <T,>({
  ...
  const { pathname, router, params, query } =
    useModelPageParams<SubTaskSearchParams>(config);

  const { limit } = params;

  const defaultFilters = getDefaultFilters(config.filters);

  const initialValues: SubTaskFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  const handleFormikSubmit = (
    values: Partial<SubTaskFormikFilter>,
    formik: FormikHelpers<SubTaskFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(
  ...
  const renderFormik = (formik: FormikProps<SubTaskFormikFilter>) => {
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as SubTaskFormikFilter);
      formik.submitForm();
    };
...
export default SubTaskFilterForm;
