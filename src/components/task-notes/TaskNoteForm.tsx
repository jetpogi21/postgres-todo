...
"use client";
import {
  TaskNoteFormFormikInitialValues,
  TaskNoteModel,
...
interface TaskNoteFormProps {
  data: TaskNoteModel | null;
  id: string;
  modalFormProps?: ModelFormProps;
  hiddenField?: string;
  onSubmit?: (
    values: TaskNoteFormFormikInitialValues,
    formik: FormikHelpers<TaskNoteFormFormikInitialValues>
  ) => void;
}
...
const TaskNoteForm: React.FC<TaskNoteFormProps> = (prop) => {
  const { id, modalFormProps, hiddenField, onSubmit } = prop;
  const { router, query, pathname } = useURL<TaskNoteSearchParams>();
  ...
  const taskNote = modelQuery.data as TaskNoteFormFormikInitialValues;

  const isLarge = useScreenSize("lg");
  const initialValues = getInitialValues<TaskNoteFormFormikInitialValues>(
    modelConfig,
    taskNote,
  ...
  const handleFormikSubmit = (
    ...
    const goToNewRecord = () => {
      formik.setValues(
        getInitialValues<TaskNoteFormFormikInitialValues>(
          modelConfig,
          undefined,
          { requiredList }
        )
    ...
    //e.g. { deletedTaskNoteTags: [], newTags: []}
  ...
  const renderFormik = (
    formik: FormikProps<TaskNoteFormFormikInitialValues>
  ) => {
