...
const TaskNoteFilterForm = <T,>({
  ...
  const { pathname, router, params, query } =
    useModelPageParams<TaskNoteSearchParams>(config);

  const { limit } = params;

  const defaultFilters = getDefaultFilters(config.filters);

  const initialValues: TaskNoteFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  const handleFormikSubmit = (
    values: Partial<TaskNoteFormikFilter>,
    formik: FormikHelpers<TaskNoteFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(
        values,
        defaultFilters
      ) as Partial<TaskNoteSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<TaskNoteFormikFilter>) => {
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as TaskNoteFormikFilter);
      formik.submitForm();
    };
...
export default TaskNoteFilterForm;
