...
const SubTaskTemplateTable = <T,>({
  ...
  const useSubTaskTemplateSearchQuery = () =>
    useModelsQuery<T>(modelConfig, {
      ...queryParams,
      fetchCount: fetchCount.toString(),
    });

  const queryResponse = useSubTaskTemplateSearchQuery();
  ...
  /* 
  const addSubTaskTemplatesFromTemplateMutation =
    useImportSubTaskTemplateFromTemplate((data) => {
      refetchQuery(0);
    });
  const modelActions = {
  ...
  */
  ...
  const handleSubmit = async (
    ...
    const rowsToBeSubmitted = (
      values[
        pluralizedModelName as keyof SubTaskTemplateFormikInitialValues
      ] as SubTaskTemplateFormikInitialValues["SubTaskTemplates"]
    ...
    asyncUpdateRecords(payload).then((data) => {
      const { inserted, updated } =
        data as unknown as UpdateModelsData<SubTaskTemplateModel>;
      ...
      Object.keys(inserted).forEach((idx) => {
        ...
        formik.setFieldValue(`${pluralizedModelName}[${idx}]`, {
          ...
          touched: false,
          [primaryKeyFieldName]:
            inserted[numIdx][primaryKeyFieldName as keyof SubTaskTemplateModel],
        });
      });
  ...
  const openDialogHandler = (row?: Row<T>["original"]) => {
    openDialog({
      ...
      message: (
        <div className="pt-8">
          <SubTaskTemplateForm
            data={(row ? row : null) as SubTaskTemplateModel | null}
            id={
              row
    ...
    (modelConfig.isTable ? (
      ...
      <Formik
        initialValues={
          ...
          } as unknown as SubTaskTemplateFormikInitialValues
        }
...
export default SubTaskTemplateTable;
