...
"use client";
import {
  SubTaskTemplateFormFormikInitialValues,
...
interface SubTaskTemplateFormProps {
  data: SubTaskTemplateModel | null;
  id: string;
  modalFormProps?: ModelFormProps;
  hiddenField?: string;
  onSubmit?: (
    values: SubTaskTemplateFormFormikInitialValues,
    formik: FormikHelpers<SubTaskTemplateFormFormikInitialValues>
  ) => void;
}
...
const SubTaskTemplateForm: React.FC<SubTaskTemplateFormProps> = (prop) => {
  const { id, modalFormProps, hiddenField, onSubmit } = prop;
  const { router, query, pathname } = useURL<SubTaskTemplateSearchParams>();
  ...
  const handleFormikSubmit = (
    ...
    //e.g. { deletedSubTaskTemplateTags: [], newTags: []}
  ...
  const renderFormik = (
    formik: FormikProps<SubTaskTemplateFormFormikInitialValues>
  ) => {
...
export default SubTaskTemplateForm;
