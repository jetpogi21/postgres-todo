...
const TaskTagFilterForm = <T,>({
  tableStates,
}: {
  tableStates: ReturnType<typeof useTableProps<T>>;
}) => {
  const config = TaskTagConfig;

  const { pathname, router, params, query } =
    useModelPageParams<TaskTagSearchParams>(config);

  const { limit } = params;

  const defaultFilters = getDefaultFilters(config.filters);

  const initialValues: TaskTagFormikFilter = getFilterValueFromURL(
    query,
    defaultFilters
  );
  ...
  const handleFormikSubmit = (
    values: Partial<TaskTagFormikFilter>,
    formik: FormikHelpers<TaskTagFormikFilter>
  ) => {
    const params = {
      ...query,
      ...(getParamsObject(
        values,
        defaultFilters
      ) as Partial<TaskTagSearchParams>),
    };
  ...
  const renderFormik = (formik: FormikProps<TaskTagFormikFilter>) => {
    ...
    const handleClearClick: MouseEventHandler = (e) => {
      e.preventDefault();
      formik.setValues(defaultFilters as TaskTagFormikFilter);
      formik.submitForm();
    };
...
export default TaskTagFilterForm;
