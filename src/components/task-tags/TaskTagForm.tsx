...
"use client";
import {
  TaskTagFormFormikInitialValues,
  TaskTagModel,
  TaskTagSearchParams,
...
interface TaskTagFormProps {
  data: TaskTagModel | null;
  id: string;
  modalFormProps?: ModelFormProps;
  hiddenField?: string;
  onSubmit?: (
    values: TaskTagFormFormikInitialValues,
    formik: FormikHelpers<TaskTagFormFormikInitialValues>
  ) => void;
}
...
const TaskTagForm: React.FC<TaskTagFormProps> = (prop) => {
  const { id, modalFormProps, hiddenField, onSubmit } = prop;
  const { router, query, pathname } = useURL<TaskTagSearchParams>();
  ...
  const taskTag = modelQuery.data as TaskTagFormFormikInitialValues;

  const isLarge = useScreenSize("lg");
  const initialValues = getInitialValues<TaskTagFormFormikInitialValues>(
    modelConfig,
    taskTag,
  ...
  const handleFormikSubmit = (
    ...
    const goToNewRecord = () => {
      formik.setValues(
        getInitialValues<TaskTagFormFormikInitialValues>(
          modelConfig,
          undefined,
          { requiredList }
        )
    ...
    //e.g. { deletedTaskTagTags: [], newTags: []}
  ...
  const renderFormik = (
    formik: FormikProps<TaskTagFormFormikInitialValues>
  ) => {
