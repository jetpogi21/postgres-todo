...
export const DELETE = async (req: Request) => {
  ...
  const body = (await req.json()) as SubTaskTemplateDeletePayload;
  const { deletedSubTaskTemplates } = body;

  if (deletedSubTaskTemplates.length > 0) {
    const { data, error } = await supabase
      .schema(AppConfig.sanitizedAppName)
      .from(modelConfig.tableName)
      .delete()
      .in(primaryKey, deletedSubTaskTemplates);
